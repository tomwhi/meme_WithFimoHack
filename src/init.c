 ...
 // print the command line 
  ...
  DO_STANDARD_COMMAND_LINE(1,
    ...
    NON_SWITCH(1, <dataset> \t\tfile containing sequences in FASTA format\n,
     ...
     DATA_OPTN(1, objfun, classic|nc|smhg|cv|nz|ll, \tobjective function (default: classic),
       obj = _OPTION_);
     ...
     DATA_OPTN(1, seed, <seed>, \t\tseed for random numbers for shuffling and \n\t\t\t\tsampling,
       seed = atoi(_OPTION_));
     DATA_OPTN(1, shuf, <kmer>, \t\tpreserve frequencies of k-mers of size <kmer> \n\t\t\t\twhen shuffling,
       kmer = atoi(_OPTION_));
     DATA_OPTN(1, ctfrac, <ctfrac>, \tfraction of control sequences to use,
