...
typedef struct options {
  ...
  // TOM: Added on April 3rd 2013, for allowing overlapping hits.
  BOOLEAN_T allowOverlap;       // Allow overlapping hits
...
static void process_command_line(
  ...
  while (TRUE) {
    ...
    else if (strcmp(option_name, "allow-overlap") == 0){
      fprintf(stderr, "TRACE: Allowing motif overlaps.\n");
...
)
{
  ...
  while (get_next_block_from_data_block_reader(fasta_reader, seq_block) != FALSE) {
    ...
    int stop = start + get_motif_length(motif) - 1;
    ...
    if (scoreable_site == TRUE) {
      ...
      // TOM: Hacking on March 28th 2013 to allow overlaps:
      if (!options->allowOverlap && pvalue > options->pthresh) {
        if (options->text_only != TRUE) {
