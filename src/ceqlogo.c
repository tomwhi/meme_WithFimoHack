...
static void LP_free(LP_PARAMS_T* params) {
  ...
  if (params->alphabet != NULL) alph_release(params->alphabet);
  memset(params, 0, sizeof(LP_PARAMS_T));
  myfree(params);
}
...
static void LP_add_line(LP_PARAMS_T* params, MATRIX_T* mat, int snum, int ltrim,
  ...
  if (params->alphabet == NULL) {
    ...
    params->alphabet = alph_hold(alphabet);
  } else if (!alph_equal(params->alphabet, alphabet)) {
    E_system("Alphabet and logo alphabet do not match!\n");
  }
  if (alph_size_core(params->alphabet) > mat->num_cols) {
...
/*
  Description   : Converts a colour in the RGB colourspace into the HSL colourspace.
                : Adaption of http://stackoverflow.com/a/9493060/66387
  Parameter     :
    r           : The red component in range 0 to 1.
    g           : The green component in range 0 to 1.
    b           : The blue component in range 0 to 1.
    h           : The hue component in range 0 to 1. OUT.
    s           : The saturation component in range 0 to 1. OUT.
    l           : The lightness component in range 0 to 1. OUT.
*/
...
/*
  Description   : Converts a colour in the HSL colourspace into the RGB colourspace.
                : Adaption of http://stackoverflow.com/a/9493060/66387
  Parameter     :
    h           : The hue component in range 0 to 1.
    s           : The saturation component in range 0 to 1.
    l           : The lightness component in range 0 to 1.
    r           : The red component in range 0 to 1. OUT.
    g           : The green component in range 0 to 1. OUT.
    b           : The blue component in range 0 to 1. OUT.
*/
...
/*
  Description   : Makes a colour lighter.
  Parameter     :
    r           : The red component in range 0 to 1.
    g           : The green component in range 0 to 1.
    b           : The blue component in range 0 to 1.
*/
static void lighten_colour(double *r, double *g, double *b) {
  double h, s, l;
  rgb2hsl(*r, *g, *b, &h, &s, &l);
  l += (1.0 - l) * 2 / 3;
  hsl2rgb(h, s, l, r, g, b);
...
/*
  Description   : Creates the colour dictionary for an alphabet
  Parameter     :
    alph        : The alphabet to create a colour dictionary for.
*/
static char* EPS_colordict(ALPH_T *alph) {
  STR_T *builder;
  int i;
  double r, g, b;
  builder = str_create(0);
  str_append2(builder, "/fullColourDict <<\n");
  for (i = 0; i < alph_size_core(alph); i++) {
    r = (double)alph_colour_r(alph, i) / 255.0;
    g = (double)alph_colour_g(alph, i) / 255.0;
    b = (double)alph_colour_b(alph, i) / 255.0;
    str_appendf(builder, " (%c)  [%.4f %.4f %.4f]\n", alph_char(alph, i), r, g, b);
  }
  str_append2(builder, ">> def\n");
  str_append2(builder, "/mutedColourDict <<\n");
  for (i = 0; i < alph_size_core(alph); i++) {
    r = (double)alph_colour_r(alph, i) / 255.0;
    g = (double)alph_colour_g(alph, i) / 255.0;
    b = (double)alph_colour_b(alph, i) / 255.0;
    lighten_colour(&r, &g, &b);
    str_appendf(builder, " (%c)  [%.4f %.4f %.4f]\n", alph_char(alph, i), r, g, b);
  }
  str_append2(builder, ">> def\n");
  return str_destroy(builder, true);
}
...
static char* EPS_data_token(LP_PARAMS_T* params) {
  ...
  STR_T *data;
  data = str_create(0);
  asize = alph_size_core(params->alphabet);
...
static RBTREE_T* EPS_tokens(LP_PARAMS_T* params) {
  ...
  // calculate the colours for the symbols
  rbtree_put(tokens, "$COLORDICT", EPS_colordict(params->alphabet));
  ...
  rbtree_put(tokens, "$BARBITS", STR_dbl(log2(alph_size_core(params->alphabet))));
  // calculate if strandedness is referenced, ie 3' and 5' or N terminus and C terminus
  // FIXME this may have to be changed to support other alphabets?
  rbtree_put(tokens, "$LOGOTYPE", (alph_has_complement(params->alphabet) ? "NA" : "AA"));
...
static void add_motif(LP_PARAMS_T* params, MOTIF_T *motif,
  ...
  assert(alph_has_complement(get_motif_alph(motif)) || !(reverse_complement));
  //FIXME I'm worried that this modification of the input motif may cause problems elsewhere.
...
static void usage() {
  printf(
   "Usage: ceqlogo [-i <filename> -m <id/n> | -i<id/n> <filename>]+ [options]\n"
